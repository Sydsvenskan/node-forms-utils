...
module.exports = function (opt) {
  ...
  f.toHTML = function (name, iterator) {
    const value = this.fieldsList
      ...
      .reduce(
        (html, field, i) => html + tag('div', { classes: ['field__multi-row'] }, field.toHTML(name + '[' + i + ']', iterator), true),
        ''
      );

    return tag(
      'div',
      { classes: ['field__multi'].concat((opt.cssClasses || {}).field || []) },
      (
        tag('div', { classes: ['field__multi-header'] }, this.labelText(name, this.id), true) +
        tag('div', { classes: ['field__multi-row-container'] }, value, true)
      ),
      true
    );
  };
